...
export default defineEventHandler(async (event) => {
	return withSpan("api.user.password.change", { "http.route": "/api/user/password" }, async () => {
		...
		if (!currentPassword || !newPassword) {
			...
			throw createError({
				statusCode: 400,
				message: "Beide Passwörter sind erforderlich",
		...
		if (!account?.password) {
			...
			throw createError({
				statusCode: 400,
				message: "Kein Passwort-Login für diesen Account",
				data: { traceId: getActiveTraceId() },
			});
		}
