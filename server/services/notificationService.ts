...
export async function notifyUser(params: NotifyParams): Promise<boolean> {
	return withSpan(
		...
		{ "notification.type": params.type, "notification.userId": params.userId },
		async (span) => {
			...
			if (params.type === "neue-ziehung" && !settings.notifyOnNewDraw) {
				log.debug("Benachrichtigung übersprungen: Neue-Ziehung-Benachrichtigung deaktiviert", {
			...
			span.setAttribute("notification.emailSent", emailSent);
			span.setAttribute("notification.slackSent", slackSent);
