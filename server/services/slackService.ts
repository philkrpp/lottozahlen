import { withSpan, injectTraceContext } from "../utils/tracing";
...
export async function sendSlackNotification(webhookUrl: string, blocks: SlackBlock[]): Promise<boolean> {
	return withSpan("external.slack.webhook", { "peer.service": "slack", "http.method": "POST" }, async (span) => {
		...
		try {
			...
			if (!response.ok) {
				log.error(`← POST Slack-Webhook ${response.status} (${duration}ms)`, {
					type: "http-out",
					method: "POST",
			...
			log.info(`← POST Slack-Webhook ${response.status} (${duration}ms)`, {
				type: "http-out",
				method: "POST",
				statusCode: response.status,
				duration,
			...
			const duration = Date.now() - start;
			log.error(`← POST Slack-Webhook NETWORK_ERROR (${duration}ms)`, {
				type: "http-out",
				method: "POST",
				duration,
				error: String(error),
