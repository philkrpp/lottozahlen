...
function getTransporter(): Transporter {
	...
	log.info("SMTP-Transporter initialisieren", {
...
export async function sendEmail(to: string, subject: string, html: string): Promise<boolean> {
	return withSpan("external.email.send", { "peer.service": "smtp", "email.subject": subject }, async (span) => {
		try {
			const config = useRuntimeConfig();
			...
			span.setAttribute("email.duration_ms", duration);
