<template>
  <div>
    ...
    <v-table v-if="sortedGewinne.length > 0" density="compact" class="endziffern-table">
      <thead v-if="!compact">
        <tr>
      ...
      <tbody>
        <tr v-for="g in sortedGewinne" :key="g.rang">
          <td>
    ...
    <div v-else class="d-flex flex-column ga-1">
      <span v-for="num in winningNumbers" :key="num" class="endziffer-nummer">
        {{ num }}
      </span>
    </div>
  </div>
