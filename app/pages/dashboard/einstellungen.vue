...
const log = createLogger("einstellungen");
...
async function handleDeleteAccount() {
	...
	log.info("Account loeschen gestartet");
	try {
		await $fetch("/api/user/account", { method: "DELETE" });
		log.info("Account geloescht");
		await signOut();
		navigateTo("/login");
	} catch {
		log.error("Account loeschen fehlgeschlagen");
