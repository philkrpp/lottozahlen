<template>
	<div>
		...
		<!-- Pagination -->
		<div
			v-if="!isLoading && draws.length > 0 && totalPages > 1"
...
const { trackEvent } = useTracking("ziehungen");
...
const selectedAnbieter = ref<string | null>("deutsche-fernsehlotterie");
const viewMode = ref<"timeline" | "table">("timeline");

const anbieterItems = computed(() => Object.values(ANBIETER).map((a) => ({ title: a.name, value: a.slug })));
